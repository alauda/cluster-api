...
type guardedClusterReconciler struct {
	detector       Detector
	controllerName string
	inner          reconcile.Reconciler
}
...
func WrapClusterReconciler(detector Detector, controllerName string, inner reconcile.Reconciler) reconcile.Reconciler {
	return &guardedClusterReconciler{
		detector:       detector,
		controllerName: controllerName,
		inner:          inner,
	}
...
func WrapClusterReconcilerWithConfigMapDetector(reader client.Reader, controllerName string, inner reconcile.Reconciler) reconcile.Reconciler {
	return WrapClusterReconciler(NewConfigMapDetector(reader), controllerName, inner)
}
...
func (r *guardedReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		// Fail closed: if standby state cannot be determined, do not mutate resources.
...
func (r *guardedClusterReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	isStandby, err := r.detector.IsStandby(ctx)
	if err != nil {
		return reconcile.Result{}, err
	}
	if isStandby && req.Name != GlobalClusterName {
		logSkip(ctx, r.controllerName, req.Name)
		return reconcile.Result{RequeueAfter: RequeueAfter}, nil
	}
	return r.inner.Reconcile(ctx, req)
}
